...
class TripletEmbedding(nn.Module):
    def __init__(
        ...
        self.sd_proj = MLP(d_sd_feats, d_g_feats, 2, activation)
        
    def forward(self, node_h, edge_h, fp, md, sd, indicators):
        ...
        triplet_h[indicators==3] = self.sd_proj(sd)
        return triplet_h
...
class LiGhTPredictor(nn.Module):
    def __init__(self,
        ...
        self.sd_predictor = nn.Sequential(
            nn.Linear(d_g_feats, d_g_feats),
            activation,
            nn.Linear(d_g_feats, d_sd_feats)
        )
