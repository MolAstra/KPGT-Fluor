...
class MoleculeDataset(Dataset):
    def __init__(
        ...
        self.cache_path = os.path.join(
            root_path, f"{dataset}/{dataset}_{path_length}.pkl"
        )
        ...
        sds = np.load(sd_path)["sd"].astype(np.float32)
        sds = torch.from_numpy(np.where(np.isnan(sds), 0, sds))

        self.df, self.fps, self.mds, self.sds = df.iloc[use_idxs], fps[use_idxs], mds[use_idxs], sds[use_idxs]
    ...
    def _pre_process(self):
        ...
        self.fps, self.mds, self.sds = self.fps, self.mds, self.sds
