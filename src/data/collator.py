...
class Collator_pretrain(object):
    def __init__(
        ...
        replace_rate=0.1,
        keep_rate=0.1,
        fp_disturb_rate=0.15,
        md_disturb_rate=0.15,
    ):
        self.vocab = vocab
    ...
    def bert_mask_nodes(self, g):
        ...
        candi_ids = np.random.choice(
            valid_ids,
            size=int(len(valid_ids) * self.candi_rate),
            replace=False,
            p=probs,
        )

        mask_ids = np.random.choice(
            candi_ids, size=int(len(candi_ids) * self.mask_rate), replace=False
        )

        candi_ids = np.setdiff1d(candi_ids, mask_ids)
        replace_ids = np.random.choice(
            candi_ids,
            size=int(len(candi_ids) * (self.replace_rate / (1 - self.keep_rate))),
            replace=False,
        )
        ...
        while np.sum(is_equal):
            new_ids[is_equal] = np.random.choice(
                valid_ids, size=np.sum(is_equal), replace=True, p=probs
            )
            new_labels = g.ndata["label"][new_ids].numpy()
            is_equal = replace_labels == new_labels
    ...
    def disturb_fp(self, fp):
        ...
        b, d = fp.shape
        fp = fp.reshape(-1)
        disturb_ids = np.random.choice(
            b * d, int(b * d * self.fp_disturb_rate), replace=False
        )
    ...
    def disturb_md(self, md):
        ...
        sampled_ids = np.random.choice(
            b * d, int(b * d * self.md_disturb_rate), replace=False
        )
...
class Collator_tune(object):
    ...
    def __call__(self, samples):
        ...
        fps = torch.stack(fps, dim=0).reshape(len(smiles_list), -1)
        mds = torch.stack(mds, dim=0).reshape(len(smiles_list), -1)
        sds = torch.stack(sds, dim=0).reshape(len(smiles_list), -1)
        ...
        return smiles_list, solvent_list, batched_graph, fps, mds, sds, labels
