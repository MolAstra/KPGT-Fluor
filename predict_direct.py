...
def get_mean_std(df_path: Path, task_name: str):
    df = pd.read_csv(df_path)
    df_train = df[df["split"] == "train"]
    std = df_train[task_name].std()
    mean = df_train[task_name].mean()
...
def finetune(args):
    ...
    model = LiGhT(
        ...
        d_sd_feats=train_dataset.d_sds,
    ...
    model.load_state_dict(
        {
            k.replace("module.", ""): v
            for k, v in torch.load(f"{args.model_path}").items()
        },
        strict=False,
    )
    ...
    logger.info(
        "model have {}M paramerters in total".format(
    ...
    trainer = Trainer(
        ...
        device=device,
        label_mean=(
            train_dataset.mean.to(device) if train_dataset.mean is not None else None
        ),
        label_std=(
            train_dataset.std.to(device) if train_dataset.std is not None else None
        ),
    ...
    best_val = trainer.predict(
        model, val_loader, save_dir=Path(args.results_dir) / "valid"
    )
    best_test = trainer.predict(
        model, test_loader, save_dir=Path(args.results_dir) / "test"
    )

    logger.info(
        f"train: {best_train[0]:.3f}, val: {best_val[0]:.3f}, test: {best_test[0]:.3f}"
    )
