...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["rocauc", "ap", "acc", "rmse", "mae", "r2", "spearman", "pearson"],
    )
    ...
    parser.add_argument("--save_dir", type=str, default="./models/downstream")
...
def finetune(args):
    ...
    logger.info(
        "Total model parameters: {:.2f}M".format(
    ...
    lr_scheduler = transformers.get_polynomial_decay_schedule_with_warmup(
        optimizer,
        args.n_epochs * len(train_dataset) // args.batch_size // 10,
        args.n_epochs * len(train_dataset) // args.batch_size,
        1e-9,
    )

    if args.metric in ["spearman", "pearson"]:
        loss_fn = SmoothL1Loss(reduction="none")
    elif args.metric in ["mae", "rmse", "r2"]:
    ...
    if args.dataset_type == "classification":
        ...
        evaluator = Evaluator(
            args.dataset, args.metric, train_dataset.n_tasks, mean=mean, std=std
        )
    ...
    trainer.fit(
        model, train_loader, val_loader, test_loader, save_dir=save_dir
    )
