...
if __name__ == "__main__":
    for fold in range(5):
        for split_method in ["random", "scaffold"]:
            for model_name in ["rf", "svr", "lightgbm", "gbrt"]:
                ...
                logger.info(f"Training {model_name} model for {split_method} split method and {fold} fold")
                save_dir = Path(
                ...
                logger.info(f"Time taken: {time.time() - time_start} seconds")
